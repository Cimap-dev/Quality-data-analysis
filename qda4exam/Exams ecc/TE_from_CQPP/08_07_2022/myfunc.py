...
def IMR_my(sample_means,n,k,mean_or_mu,sigma_v,mean_to_est,sigma_v_to_est,use_means_as_I='no'):

    df = pd.DataFrame({'I': sample_means})

    if use_means_as_I!='no': #yes
    ...
    if mean_to_est=='yes' and sigma_v_to_est=='yes':
        ...
        df['I_LCL'] = mean_or_mu - (k*sigma_v)
        if use_means_as_I!='no':
            df['MR_UCL'] = d2*np.sqrt(n) + k*d3*sigma_v*np.sqrt(n)
            df['MR_CL'] = d2*sigma_v*np.sqrt(n)
            df['MR_LCL'] = np.max(0,d2**np.sqrt(n) - k*d3*sigma_v*np.sqrt(n))
...
def reconstruct(data,pca,scores_df,k):
    mean = data.mean()
    std = data.std()
    ...
    reconstructed_data_std = scores_df.iloc[0:,0:k].dot(pca.components_[0:k, :])
...
def boxcox_and_unstack(data):
    ...
    if (lmbda <= 0.050 and lmbda>=-0.050):
        lmbda=0
...
def IMR_TrendCC(data_IMR, model, datapositive='no'):
    d2 = qda.constants.getd2(2)
    ...
    data_IMR['I_UCL'] = data_IMR['I_CL'] + 3 * df_res['MR'].mean() / d2
    data_IMR['I_LCL'] = data_IMR['I_CL'] - 3 * df_res['MR'].mean() / d2
...
def chi2cc(p,m,n,alpha,data,mu,SIGMA):
