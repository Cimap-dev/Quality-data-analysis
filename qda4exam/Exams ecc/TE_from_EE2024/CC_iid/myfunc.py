...
def xbars_mu_given (mu,std_or_sigma,n,alpha,sample_mean, sample_std):
...
def xbarR(mean_or_mu,std_or_sigma,n,alpha,sample_mean, sample_R):
    K=stats.norm.ppf(1-alpha/2)
...
def boxcox_and_unstack(data,approx_to_0_if_nec='False'):
    ...
    if (lmbda < 0.10 and lmbda>-0.10):
        if(approx_to_0_if_nec!='False'):
            lmbda=0
            data_BC=(data.stack()).transform(lambda x: ((np.log(x))))
            print('ma il lambda usato è 0')
...
def IMRcc_half_normal(data,nome_colonna_dati,alpha=0.0027):
...
def IMRccboxcox(data, nome_colonna_dati, alpha=0.0027):
    K_alpha = stats.norm.ppf(1-alpha/2)
    data_IMR = qda.ControlCharts.IMR(data, nome_colonna_dati, K = K_alpha)
...
def IonMRresBoxCox(df_SCC,use04='no'):
...
def T2cc_n_1_ph2(p,m,m_ph2,alpha,data,Xbar,S2):
